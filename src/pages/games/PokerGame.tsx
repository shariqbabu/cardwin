...
const PlayerSeat: React.FC<{
  ...
  return (
    <div
      className={`flex flex-col items-center gap-0.5 transition-opacity duration-300 ${folded ? 'opacity-50' : 'opacity-100'}`}
    ...
    >
      ...
      <div className="relative" style={{ zIndex: 20 }}>
        ...
        {isActive && isPlaying && turnSecondsLeft !== undefined && (
          <svg className="absolute inset-0 pointer-events-none" style={{ zIndex: 11, width: '100%', height: '100%' }} viewBox="0 0 56 56">
            <circle
              cx="28" cy="28" r="26"
              fill="none"
              ...
              strokeDashoffset={`${2 * Math.PI * 26 * (1 - turnSecondsLeft / 30)}`}
              strokeLinecap="round"
...
const PokerGamePage: React.FC = () => {
  ...
  // Guard: settle called only once per showdown
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      // ✅ Server-side auto-fold via API (only if it's my turn that expired)
      if (remaining === 0 && tableId && table?.activePlayerUid === user?.uid) {
        apiPokerAction(tableId, 'fold').catch(() => {});
      }
    };
    tick();
  ...
  // ── Auto-start: 2 players joined → start after 2s ─────────────────────────
  useEffect(() => {
    ...
    if (
      ...
      setTimeout(async () => {
        try { await apiPokerStart(tableId); } catch {}
  ...
  // ── Auto settle on showdown → then next hand after 4s ─────────────────────
  useEffect(() => {
    if (!table || !tableId) return;
    if (table.phase === 'showdown' && table.status === 'waiting') {
      // settle once per unique showdown
      const settleKey = `${tableId}-${table.handNumber}`;
      if (settledRef.current !== settleKey) {
        settledRef.current = settleKey;
        apiPokerSettle(tableId).catch(() => {});
      }

      // Next hand after 4s if enough players remain
      if ((table.players.length ?? 0) >= 2) {
  ...
  }, [table?.phase, table?.status, table?.handNumber]);

  // ── Navigate away if broke ─────────────────────────────────────────────────
  useEffect(() => {
    if (!table || !user) return;
  ...
  const winnerUids = React.useMemo(() => {
    ...
    const brokeUids      = new Set((table.lastBrokePlayers || []).map((b: { uid: string }) => b.uid));
    const activePlayers  = table.players.filter(p => p.status !== 'folded' && !brokeUids.has(p.uid));
    ...
    if (activePlayers.length > 1) {
      ...
      return new Set(activePlayers.filter(p => p.chips === topChips).map(p => p.uid));
    }
  ...
  const handleAction = async (action: 'fold' | 'check' | 'call' | 'raise' | 'allin') => {
    ...
    try {
      await apiPokerAction(tableId, action, action === 'raise' ? raiseAmount : undefined);
    } catch (e: any) {
  ...
  // ── Arrange players: me = displaySeat 0 ───────────────────────────────────
  ...
  if (!table) return (
    <div className="h-screen flex items-center justify-center text-white" style={{ background: 'linear-gradient(180deg,#1a0a2e,#080e1a)' }}>
      <div className="text-center space-y-3">
        ...
        <button onClick={() => navigate('/games/poker')} className="text-yellow-400 text-sm underline">← Back to Lobby</button>
      </div>
    </div>
  );
  ...
  // ── Render ─────────────────────────────────────────────────────────────────
  return (
    <div className="fixed inset-0 flex flex-col overflow-hidden select-none" style={{ background: 'linear-gradient(180deg,#12082a 0%,#0a1020 60%,#060810 100%)' }}>

      {/* ════════ HEADER ════════ */}
      <div className="shrink-0 flex items-center justify-between px-3 md:px-6 py-2.5 border-b border-white/5 z-50" style={{ background: 'rgba(8,6,20,0.97)' }}>
        <button
        ...
        <div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center">
          ...
          {(table.sidePots || []).length > 1 && (
            <span className="text-gray-500 text-[9px]">{table.sidePots.length} side pots</span>
          )}
        </div>
      ...
      <div className="shrink-0 flex justify-center pt-2 pb-1 z-50">
        <span className={`text-[10px] md:text-xs font-black tracking-[2px] uppercase px-5 py-1.5 rounded-full border ${
          phase === 'waiting'
          ...
          {phase === 'waiting' ? `WAITING FOR PLAYERS ${numPlayers}/2` : phase.toUpperCase()}
        </span>
      </div>

      {/* ════════ TABLE AREA ════════ */}
      <div className="flex-1 flex justify-center items-start overflow-visible" style={{ minHeight: 0, paddingTop: '40px' }}>
        <div
          className="relative mx-auto"
          style={{
            width: 'min(100vw, 560px)',
            height: showActions ? '62vh' : phase === 'showdown' ? '74vh' : '72vh',
          }}
        >
          {/* GREEN FELT */}
          <div
            ...
            style={{
              ...
              borderRadius: '30%',
              background: 'radial-gradient(ellipse at 50% 38%,#2a9a5a 0%,#1a7a40 45%,#0f5a28 80%,#082010 100%)',
              boxShadow: '0 0 0 5px #c8922a, 0 0 0 10px #8a5a18, 0 0 0 13px #5a3a0a, 0 25px 80px rgba(0,0,0,0.9)',
              overflow: 'visible',
              zIndex: 1,
            }}
          >
            <div className="absolute inset-0 pointer-events-none" style={{ borderRadius: '40%', boxShadow: 'inset 0 0 60px rgba(0,0,0,0.5)' }} />

            {/* Spade watermark */}
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none" style={{ color: 'rgba(255,255,255,0.03)', fontSize: 'clamp(60px,15vw,110px)', zIndex: 2 }}>
              ♠
            </div>

            {/* CENTER CONTENT */}
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 px-6" style={{ zIndex: 3 }}>
              {phase !== 'waiting' && (
              ...
              {phase === 'showdown' && (
                <p className="text-yellow-400 font-black text-sm md:text-base animate-bounce">🏆 Showdown!</p>
              )}
            </div>
          </div>
      ...
      {/* ════════ BOTTOM ACTION PANEL ════════ */}
      <div className="shrink-0 z-40 border-t border-white/5" style={{ background: 'rgba(6,8,18,0.98)' }}>
        <div className="max-w-lg mx-auto px-2 pt-0.5 pb-0.5">
            ...
            <div className="flex items-center justify-center gap-2 py-2">
              ...
              <p className="text-gray-400 text-sm">
                ...
                <span className="text-blue-400 font-semibold">
                  {spectatorCount > 1 ? `${spectatorCount - 1} others waiting ahead` : "You're next in queue"}
                </span>
              </p>
            </div>
          )}
          ...
          {phase === 'waiting' && myRole === 'player' && (
            <div className="flex flex-col items-center gap-2">
              {canStart ? (
                <button
                  onClick={() => tableId && apiPokerStart(tableId)}
            ...
            <div className="space-y-1">
              ...
              {showRaise && (myPlayer?.chips || 0) > callAmount && (
                <div className="bg-white/5 border border-white/10 rounded-xl p-2 space-y-1">
                  ...
                  <input
                    type="range"
                    min={minRaise} max={maxRaise} step={table?.bigBlind || 10}
              ...
              {(myPlayer?.chips || 0) > callAmount && (
                ...
                >
                  {showRaise ? `✓ Confirm Raise — ${formatCurrency(Math.min(raiseAmount, maxRaise))}` : '↑ Raise'}
                </button>
              )}
          ...
          {myPlayer?.status === 'folded' && phase !== 'showdown' && (
            <p className="text-center text-red-400/70 text-sm py-2">You folded — watching the hand...</p>
          )}

          {/* All in */}
          {myPlayer?.status === 'allin' && phase !== 'showdown' && (
            <p className="text-center text-yellow-400 font-black text-sm animate-pulse py-2">ALL IN 🎯 — Waiting for showdown...</p>
          )}
      ...
      {/* ════════ LEAVE MODAL ════════ */}
      {showLeave && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md flex items-end md:items-center justify-center z-[100] px-4" onClick={() => setShowLeave(false)}>
          <div className="bg-[#0d1520] border border-white/10 p-6 w-full max-w-sm text-center rounded-t-2xl md:rounded-2xl" onClick={e => e.stopPropagation()}>
            <div className="w-10 h-1 bg-white/20 rounded-full mx-auto mb-5 md:hidden" />
            <LogOut className="w-8 h-8 text-red-400 mx-auto mb-3" />
            <h3 className="text-white font-black text-lg mb-1" style={{ fontFamily: 'Georgia, serif' }}>Leave Table?</h3>
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowLeave(false)} className="flex-1 bg-white/8 border border-white/12 text-white font-bold py-3.5 text-sm rounded-xl hover:bg-white/12 transition-colors">
                Stay
              </button>
              <button
                onClick={handleLeave}
